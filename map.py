...
class Map:
    ...
    def __init__(self, screen, settings, bricks, pipes, mario, enemies, ground, upgrades, stats, secret_bricks):
        ...
        self.ground = ground  # +
        self.main_level = f'images/level_loc.txt'
        # self.secret_level = f'images/Underground_Level.txt'
    ...
    def build_brick(self):
        ...
        for n_row in range(len(self.rows)):
            row = self.rows[n_row]
            for n_col in range(len(row)):
                col = row[n_col]
                if col == 'B':
                    Map.create_brick(self, n_col * dx, n_row * dy, 0)
                if col == '?':
                    Map.create_brick(self, n_col * dx, n_row * dy, 1)
                if col == 'X':
                    Map.create_brick(self, n_col * dx, n_row * dy, 3)
                if col == 'M':
                    Map.create_brick(self, n_col * dx, n_row * dy, 2)
                if col == 'R':
                    Map.create_brick(self, n_col * dx, n_row * dy, 4)
                if col == 'S':
                    Map.create_brick(self, n_col * dx, n_row * dy, 5)
                if col == 'I':
                    Map.create_brick(self, n_col * dx, n_row * dy, 6)
                if col == 'G':
                    Map.create_enemy(self, n_col * dx, n_row * dy, 0)
                if col == 'K':
                    Map.create_enemy(self, n_col * dx, n_row * dy, 1)
                if col == 'A':
                    Map.create_secret_brick(self, n_col * dx, n_row * dy, 7)
                if col == 'N':
                    Map.create_secret_brick(self, n_col * dx, n_row * dy, 7)
                if col == 'C':
                    Map.create_coin(self, n_col * dx, n_row * dy, 4)
                if col == 'L':
                    Map.create_brick(self, n_col * dx, n_row * dy, 0)
